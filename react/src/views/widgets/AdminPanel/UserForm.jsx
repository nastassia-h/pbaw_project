...
const UserForm = ({user = {}, type, setData}) => {
   ...
   const submit = (values, onSubmitProps) => {
      ...
      if (type === 'create') {
         ...
         .then(( response ) => {
            if (response && response.status === 201) {
               ...
               dispatch(setNotification('User was successfully created'));
            }
         })
         .catch(err => {
      ...
      if (type === 'update') {
         ...
         .then(({ data }) => {
            setData(data)
            dispatch(setNotification('User was successfully updated'));
         })
         .catch(err => {
            const response = err.response;
            if (response && response.status === 422) {
      ...
      console.log('invalide form type') 
   }
   ...
   return (
      <Box width="95%" p="1rem" m="0 auto" borderRadius="1.5rem" textAlign={'center'}>
         <>
            <Formik
